define(['objects/canvas/canvas'], function(canvas){

    function imageRepeat (imageEl, startX, startY, containerWidth, containerHeight) {
        ...
        for (var i=0; i <= repeatTimesVert; i++){
            for (var j=0; j <= repeatTimesHoriz; j++){
                ...
                if (i === repeatTimesVert && j === repeatTimesHoriz) {
                    canvas.ctx.drawImage(imageEl, 0, 0, widthModulus, heightModulus, x, y, widthModulus, heightModulus);
                    // Last row. Cut height
                } else if (i === repeatTimesVert) {
                    canvas.ctx.drawImage(imageEl, 0, 0, width, heightModulus, x, y, width, heightModulus);
                    // Last row item. Cut width
                } else if (j === repeatTimesHoriz) {
                    canvas.ctx.drawImage(imageEl, 0, 0, widthModulus, height, x, y, widthModulus, height);
                    // Full image
                } else {
                    canvas.ctx.drawImage(imageEl, x, y);
                }
